...
export default function InvoiceGenerator() {
  ...
  const [invoiceNumber, setInvoiceNumber] = useState(() => getNextInvoiceNumber(invoices, userId))
  const [issueDate, setIssueDate] = useState(new Date().toISOString().slice(0, 10))
  const [dueDate, setDueDate] = useState(createDefaultDueDate())
  const [pricingMode, setPricingMode] = useState<'excl' | 'incl'>('excl')
  const [noVat, setNoVat] = useState(false)
  const [currencyCode, setCurrencyCode] = useState('EUR')
  const [clientName, setClientName] = useState('')
  const [clientEmail, setClientEmail] = useState('')
  const [companyName, setCompanyName] = useState('SmartInvoice Demo B.V.')
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-100 via-cyan-50 to-white px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-cyan-700">Fase 1 Prototype</p>
          <h1 className="mt-2 text-3xl font-extrabold leading-tight sm:text-4xl">SmartInvoice Generator</h1>
          <p className="mt-3 max-w-3xl text-sm text-slate-600 sm:text-base">
            Dit is de eerste werkende versie: client-side formulier, automatische BTW-berekening (21/9/0),
            en live factuurpreview.
          </p>
                ...
                {lines.map((line) => (
                  <div key={line.id} className="grid gap-2 rounded-xl border border-slate-200 p-3 sm:grid-cols-12">
                    <input
                      ...
                      className="rounded-lg border border-slate-300 px-2 py-2 text-sm sm:col-span-4"
                    />
                    <input
                      type="number"
                      min="0"
                    ...
                    <button
                      ...
                      className="rounded-lg border border-rose-200 px-2 py-2 text-sm font-medium text-rose-600 transition hover:bg-rose-50 sm:col-span-2"
                    >
                      Verwijder
                    </button>
                  </div>
