...
export default function SupportPage() {
  ...
  const submitTicket = async () => {
    ...
    if (normalizedSubject.length < 3) {
      setTicketError('Onderwerp moet minimaal 3 tekens zijn.')
      return
    }

    if (normalizedMessage.length < 10) {
      setTicketError('Bericht moet minimaal 10 tekens zijn.')
      return
    }
    ...
    if (error) {
      setTicketError('Ticket versturen is mislukt. Probeer het opnieuw of gebruik e-mail.')
      return
    }
    ...
    setTicketStatus('Je supportticket is verzonden. We reageren zo snel mogelijk.')
    setSubject('')
    setMessage('')
    await loadMyTickets()
  }
    ...
    <main className="space-y-4">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-700">Support</p>
            <h1 className="mt-2 text-2xl font-extrabold text-slate-900">Hulpcentrum</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              Vind snel antwoorden, bekijk de meest gebruikte onderwerpen en neem contact op als je er niet uitkomt.
            </p>
          </div>
          <a
            href={contactHref}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-700">Mijn tickets</p>
            <p className="mt-1 text-sm text-slate-600">Bekijk de status van je laatste supportvragen en reacties van admin.</p>
          </div>
          <button
            type="button"
            onClick={() => {
              ...
              <article key={ticket.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                ...
                {ticket.admin_response ? (
                  <div className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-700">Admin reactie</p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <label className="w-full sm:max-w-lg">
            <span className="mb-1 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Zoeken</span>
            <input
              type="text"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Bijv. BTW, factuurstatus of import"
        ...
        <div className="mt-5 grid gap-3">
          {filtered.length === 0 ? (
            <p className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
              Geen resultaten gevonden. Probeer een ander zoekwoord of neem contact op met support.
            </p>
          ) : (
            filtered.map((article) => (
              <details key={article.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-3 flex items-center gap-2 border-t border-slate-200 pt-3">
                  <span className="text-xs font-semibold text-slate-500">Was dit nuttig?</span>
                  <button
                    type="button"
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-700">Support ticket</p>
            <h2 className="mt-1 text-lg font-bold text-slate-900">Kom je er niet uit? Stuur ons een bericht</h2>
          </div>
          <a
            href={contactHref}
          ...
          >
            Of mail direct naar info@webdesignerstudio.nl
          </a>
        </div>

        <div className="mt-3 grid gap-3">
          <label className="flex flex-col gap-1">
            <span className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Onderwerp</span>
            <input
              type="text"
              value={subject}
              onChange={(event) => setSubject(event.target.value)}
              placeholder="Bijv. Factuurstatus wijzigt niet"
              className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-cyan-500 focus:outline-none"
            />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Bericht</span>
            <textarea
              value={message}
              onChange={(event) => setMessage(event.target.value)}
              rows={5}
              placeholder="Omschrijf je vraag, wat je deed en wat je verwachtte."
              className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-cyan-500 focus:outline-none"
            />
          </label>
          ...
          <div>
            ...
            >
              {isSubmittingTicket ? 'Versturen...' : 'Verstuur ticket'}
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-700">Snel naar</p>
